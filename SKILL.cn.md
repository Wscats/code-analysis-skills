...
只要其中任何一条不清晰，**代理必须拒绝执行**，并请用户明确澄清。CLI 与 Skill 入口也用硬性门槛强制此政策（必须传 `--i-have-consent` 或 `acknowledge_usage_policy: true`，或设置 `CODE_ANALYSIS_ACK_USAGE_POLICY=1` 环境变量）。
...
1. **🪞 自查摘要** — 综合描述性指标（0–100），指标分段（S/A/B/C/D/E/F），支持性观察、值得讨论的点（带上下文）、讨论提示。**不是**评分或定论。
2. **📉 节奏密度信号** — 描述 Git 活动有多稀疏 / 集中 / 低量。**不是**生产力或敬业度的衡量。许多正常工作模式都会产生稀疏的提交节奏。
...
多人分析时会附一份**按字母顺序排列的总览**（**有意不做排行榜**）。请不要把这些表格当作排名展示给用户。
...
综合指标是对 Git 历史多个维度的**描述性**汇总，**不是**衡量人的价值、能力或表现。

| 分段 | 指标范围 | 含义（仅描述性） |
|------|----------|------------------|
| S | 90–100 | 此样本中绝大多数 Git 历史维度看起来健康。 |
| A | 80–89  | 多数维度健康，少量值得聊一聊。 |
| B | 70–79  | 多数维度健康，有几项值得 1:1 时讨论。 |
| C | 60–69  | 表现不一，多个维度可能值得讨论。 |
| D | 50–59  | 中等指标；具体维度可能值得 1:1。 |
| E | 35–49  | 多个指标低于常见范围 — 通常说明 Git 看不到的因素（角色侧重、休假、阻塞）。 |
| F | 0–34   | 多维度显著偏低 — 常见原因是值班 / 非编码工作、休假等 Git 历史不可见的情况。 |

### 六大维度（描述性权重）

| 维度 | 权重 | 描述内容 |
|------|------|----------|
| 📝 提交纪律 | 15% | 提交频率、消息长度、规范遵循率 |
| ⏰ 节奏一致性 | 15% | 提交时间戳的分布 |
| 🚀 变更模式 | 20% | Churn、Rework、变更量 |
| 🔍 代码质量痕迹 | 25% | Bug 修复率、回滚率、变更中的测试覆盖、复杂度 |
...
| 分段 | 指标范围 | 含义 |
|------|----------|------|
| 活跃密集 | 0–20 | 在活跃跨度内提交频繁。 |
| 规律活跃 | 21–40 | 典型的提交节奏。 |
| 节奏不均 | 41–60 | 节奏不均，期间有较长的安静段。 |
| 节奏稀疏 | 61–80 | 安静段较多；信号是局部的，需要结合上下文。 |
| 节奏非常稀疏 | 81–100 | 活动集中在短时间内。请结合完整上下文阅读。 |

> **重要**：节奏稀疏度高**不**等于"摸鱼"——只是说明提交活动在时间上很集中。许多合理的角色与生活情况都会产生这种模式。
