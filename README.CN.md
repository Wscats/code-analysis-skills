...
- 🪞 **自查摘要** — 描述性指标（不是评分），支持性观察、值得讨论的点、讨论提示
- 📋 **多人对比总览** — **按字母顺序**排列（有意不做排行榜）
...
### 6. 📉 节奏密度信号（仅描述性）
- 综合多个信号描述提交活动**有多稀疏 / 集中**（0–100）
- 信号包括：稀疏度、琐碎提交、长间隔、低日量、非代码提交、周内偏后、增删失衡
- 分段（**仅描述性**）：活跃密集 → 规律活跃 → 节奏不均 → 节奏稀疏 → 节奏非常稀疏

> ⚠️ 高稀疏度 **不等于** "摸鱼"。架构师、Reviewer、值班工程师、休假 / 病假人员都会自然产生稀疏节奏。请结合上下文阅读。

### 7. 🪞 自查摘要
- 六维度加权综合**描述性指标**（提交纪律 15% + 节奏一致性 15% + 变更模式 20% + 代码质量痕迹 25% + 风格 10% + 节奏密度 15%）
- 指标分段：S / A / B / C / D / E / F（**仅描述性**，不是对人的评级）
- 支持性观察、值得讨论的点（带上下文）、讨论提示
- 一句话中性总结
...
**强制门槛**：CLI 必须传 `--i-have-consent`，Skill 必须设置 `acknowledge_usage_policy: true`，或设置 `CODE_ANALYSIS_ACK_USAGE_POLICY=1` 环境变量，否则工具直接拒绝运行。
