...
## 📉 节奏稀疏度指标（Cadence-Sparsity Indicator，原“摸鱼指数”）

节奏稀疏度是一个描述性综合评分 (0-100)，描述 Git 提交活动**有多稀疏 / 集中**。

> ⚠️ 该指标**仅描述性**。高值不等于“摸鱼”，也不等于低投入度。架构师、Reviewer、值班工程师、休假 / 病假人员，都会自然产生稀疏节奏。
> 本指标**不应**用于绩效考核、排名或任何 HR 决策。
...
| Trivial-change ratio (琐碎提交) | 变更 ≤5 行的提交占比 | trivial_count / total | 20 |
| Long-gap ratio (长间隔) | 超72小时无提交的间隔占比 | large_gap_count / total_gaps | 20 |
| Low daily volume (低日量) | 每活跃日的变更行数 | total_lines / unique_days | 15 |
| Non-code-only (仅非代码提交) | 仅修改配置/文档的提交占比 | non_code_commits / total | 10 |
| Late-week skew (周内偏后) | 周五提交多、周一提交少的趋势 | friday_ratio - monday_ratio | 10 |
| Add/delete imbalance (增删失衡) | 新增/删除行比率过高 | added / deleted > 10 | 5 |

### 分段（描述性）

| 分段 | 分数范围 | 含义 |
|------|----------|------|
| Dense activity (活跃密集) | 0-20 | 在活跃跨度内提交频繁 |
| Regular activity (规律活跃) | 21-40 | 典型的提交节奏 |
| Mixed cadence (节奏不均) | 41-60 | 节奏不均，期间有较长的安静段 |
| Sparse cadence (节奏稀疏) | 61-80 | 安静段较多；信号是局部的，需要结合上下文 |
| Very sparse cadence (节奏非常稀疏) | 81-100 | 活动集中在短时间内，请结合完整上下文阅读 |

### 重要提醒

- 该指标仅基于 Git 提交行为，不反映代码评审、设计、会议、指导同事、值班、运维等非编码贡献
- 某些角色（架构师、技术经理、Reviewer）天然提交较少，需结合角色理解
- **该指标绝不应用于绩效考核、个人排名或惩罚性管理**

## 🪞 自查报告体系

### 综合描述性指标 (0-100)

综合指标由六大维度加权计算得出。该指标**仅描述**本样本中 Git 历史的一般表现，**不是**对人的评价。

| 维度 | 权重 | 评估内容 |
|------|------|----------|
| Commit Discipline (提交纪律) | 15% | 提交频率、消息长度、规范遵循率 |
| Cadence Consistency (节奏一致性) | 15% | 提交时间戳的分布 |
| Change Patterns (变更模式) | 20% | 代码流失率、返工率、每次提交的变更量 |
| Code Quality artefacts (代码质量痕迹) | 25% | Bug修复率、回滚率、变更中的测试覆盖、复杂度 |
| Code Style (代码风格) | 10% | Conventional Commits 遵循率、Issue引用率 |
| Cadence Density (节奏密度) | 15% | 节奏稀疏度信号的反向 (100 - sparsity_indicator) |

### 分段对照表（描述性）

| Band | 分数 | 含义（仅描述性） |
|-------|------|-------------------|
| S | 90-100 | 本样本中多个维度看起来都健康 |
| A | 80-89  | 多个维度健康，少量值得讨论 |
| B | 70-79  | 多数维度健康，几项值得 1:1 聊一聊 |
| C | 60-69  | 表现不一，多个维度可能值得讨论 |
| D | 50-59  | 中等指标；具体维度可能值得 1:1 |
| E | 35-49  | 多个指标低于常见范围 — 通常说明 Git 看不到的因素 |
| F | 0-34   | 多维度显著偏低 — 常见原因是值班 / 非编码工作、休假等 Git 不可见的情况 |

### 报告输出

每位开发者的自查包含：

1. **综合描述性指标**：0-100 的数值 + S/A/B/C/D/E/F 分段（**仅描述性**）
2. **六维度分数**：每个维度 0-100 分，带可视化条形图
3. **支持性观察**：每条基于具体数据
4. **值得讨论的点（带上下文）**：中性、不带个人贬损
5. **讨论提示**：可以拿去 1:1 谈的问题
6. **中性一句话总结**
